...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--output' && i + 1 < args.length) {
      ...
      config.includeLayers = args[++i]
        .split(',')
        .map((s) => s.trim())
        .filter((s) => s);
    } else if (arg === '--exclude-layers' && i + 1 < args.length) {
      config.excludeLayers = args[++i]
        .split(',')
        .map((s) => s.trim())
        .filter((s) => s);
...
function generateQueryFromLayers(
  areaId,
  bbox = null,
  includeLayers = null,
  excludeLayers = null,
  ...
  if (includeLayers && includeLayers.length > 0) {
    ...
    if (beforeCount !== afterCount) {
      console.log(
        `   ℹ️  Whitelist applied: ${afterCount} layer(s) included (from ${beforeCount} total)`
      );
    }
  }
  ...
  if (excludeLayers && excludeLayers.length > 0) {
    ...
    if (beforeCount !== afterCount) {
      console.log(
        `   ℹ️  Blacklist applied: ${beforeCount - afterCount} layer(s) excluded (${afterCount} remaining)`
      );
    }
  }
          ...
          res.on('end', () => {
            try {
              ...
              if (relation.osm_type === 'relation') {
                ...
                resolve({
                  areaId,
                  relationId: relation.osm_id,
                  displayName: relation.display_name,
                });
              } else {
                reject(
                  new Error(
                    `No relation found for area: ${areaName}. Found ${relation.osm_type} instead.`
                  )
                );
              }
            } catch (e) {
...
async function executeOverpassQueryWithFallback(query, endpoint, spinner = null) {
  ...
  for (let i = 0; i < OVERPASS_SERVERS.length; i++) {
    ...
    try {
      ...
      if (i > 0) {
        console.log(
          `   ✓ Successfully used fallback server ${serverNumber}/${OVERPASS_SERVERS.length}: ${server}`
        );
      } else {
        console.log(
          `   ✓ Successfully used server ${serverNumber}/${OVERPASS_SERVERS.length}: ${server}`
        );
      }
