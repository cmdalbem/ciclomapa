import { calculateCyclepathCoverage } from './utils/geojsonUtils.js';
import { HYBRID_MAX_RESULTS } from './config/constants.js';
import {
  getRouteScore,
  getCoverageBreakdown,
  getCoverageBreakdownSimple,
...
class DirectionsManager {
      ...
      if (directions && directions.routes && directions.routes.length > 0) {
        ...
        routesWithScores = directions.routes.map((route, index) => {
          ...
          if (route.geometry && route.geometry.type === 'LineString' && hasCoverageData) {
            ...
            console.debug(
              `Route ${index} cyclepath coverage calculated:`,
              coverageResult.coverage + '%'
            );
            console.debug(`Route ${index} coverage by type:`, coverageResult.coverageByType);
            console.debug(
              `Route ${index} overlap segments found:`,
              coverageResult.overlappingCyclepaths.length
            );
          }
  ...
  async calculateHybridDirections(fromCoords, toCoords) {
    ...
    console.debug(
      `Hybrid calculation completed: ${allRoutes.length} routes from ${results.filter((r) => r.status === 'fulfilled' && r.value.routes.length > 0).length} providers`
    );
