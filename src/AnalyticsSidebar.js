...
import { MdDataUsage as IconAnalytics } from 'react-icons/md';

import { removeAccents } from './utils.js';
import AirtableDatabase from './AirtableDatabase.js';

import { LENGTH_CALCULATE_STRATEGIES } from './constants.js';
    ...
    if (this.state.allMetadata && this.state.allMetadata.length > 0) {
      search = this.state.allMetadata.find((v) =>
        this.props.location
          .toLowerCase()
          .removeAccents()
          .includes(v.fields.location.toLowerCase().removeAccents())
      );
      this.setState({
  ...
  render() {
    ...
    return (
      <div
        id="analyticsSidebar"
        className={`
                    background-black border-l border-opacity-10 border-white h-screen ${
                      this.state.open ? 'w-60 overflow-y-auto flex-none' : ''
                    }
                    transform transition-transform duration-500 ${
                      this.state.open ? '' : 'translate-x-full'
                    }`}
        ...
        <div className="px-4">
          <div className="flex w-full justify-between items-center pt-2 mt-1">
            <div className="flex items-center">
              ...
              <h2 className="my-0 pl-1">Métricas</h2>
            </div>
          ...
          {this.state.cityMetadata &&
            this.state.cityMetadata.pnb_total !== undefined && (
              <Section
                title="PNB"
                ...
                description={
                  <>
                    <p>
                      O People Near Bike (pessoas próximas a bicicleta) é
                      apurado anualmente pelo ITDP Brasil para avaliar as
                      políticas de ciclomobilidade com maior efetividade e
                      indica o percentual de pessoas que moram a até 300 metros
          ...
          {this.state.cityMetadata &&
            this.state.cityMetadata.ideciclo !== undefined && (
              <Section
                title="IDECiclo"
                ...
                description={
                  <>
                    <p>
                      O Índice de Desenvolvimento Cicloviário (IDECICLO) tem
                      como objetivo avaliar qualitativamente a infraestrutura
                      cicloviária da cidade de forma objetiva e replicável de
                      modo que haja acompanhamento da evolução dos parâmetros
                      para uma comparação entre infraestruturas e entre cidades,
                      de maneira a construir uma séria histórica.
                    </p>
                    <p>
                      O IDECICLO faz uma avaliação local de cada estrutura e
                      pondera esta sob a malha total da cidade e com as
                      velocidades máximas das vias que estão sendo inseridas,
                      podendo ser um índice comparativo entre cidades e entre
                      estruturas.
                    </p>
                    <p>Escala: de 0 a 1.</p>
                  </>
          ...
          <Section
            ...
            description={
              <>
                <p>
                  As extensões totais das vias são calculadas automaticamente
                  com base nos dados do OpenStreetMap.
                </p>
                <p>
                  Para vias que tem estrutura dos dois lados nós desenvolvemos
                  um método que automaticamente detecta estes casos e remove
                  esta contagem dupla do total.
                </p>
                <p className="italic opacity-50 text-xs">
                  Estes números podem não corresponder à realidade. Eles não são
                  dados oficiais e dependem do estado atual do mapeamento da
                  infraestrutura da cidade no OpenStreetMap e da precisão do
                  nosso método de detecção automática de contagens duplas.
                </p>
              </>
            }
          >
              ...
              {layers
                ...
                .map((l) => (
                  <DataLineWithBarChart
                    ...
                    percent={
                      lengths && (lengths[l.id] * 100) / this.state.totalLength
                    }
          ...
          <Section
            ...
            description={
              <>
                <p>
                  Contagem total dos pontos de interesse do ciclista de cada um
                  dos tipos com que o CicloMapa trabalha.
                </p>
                <p className="italic opacity-50 text-xs">
                  Estes números podem não corresponder à realidade. Eles não são
                  dados oficiais e dependem do estado atual do mapeamento da
                  infraestrutura da cidade no OpenStreetMap.
                </p>
              </>
            }
          >
