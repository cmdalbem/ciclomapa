...
import { doesAContainsB, downloadObjectAsJson } from './utils.js'
...
class Map extends Component {
    ...
    updateDebugPolygon(bbox, id) {
        const polygon = this.createPolygonFromBBox(bbox);
        console.log('polygon', polygon);

        map.getSource('debug-polygon-' + id).setData(polygon);
    }

    createPolygonFromBBox(bbox) {
        return {
            'type': 'Feature',
            'geometry': {
                'type': 'Polygon',
    ...
    update() {
        return new Promise((resolve, reject) => {
            ...
            currentBBox = this.getCurrentBBox();
            const query = this.getQuery(currentBBox);
    ...
    initLayers() {
        ...
        map.on("mouseenter", "cycleways-lines", e => {
        ...
        map.on("mouseleave", "cycleways-lines", e => {
            // Hover style
